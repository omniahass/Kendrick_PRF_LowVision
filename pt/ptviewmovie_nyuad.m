function [timeframes,timekeys,digitrecord,trialoffsets] = ...
  ptviewmovie_nyuad(images,frameorder,framecolor,frameduration,fixationorder,fixationcolor,fixationsize, ...
              ...
              frameevents,framefuncs,setupscript,cleanupscript,stereocontrol,stereoflip,setnum)

% function [timeframes,timekeys,digitrecord,trialoffsets] = ...
%   ptviewmovie_nyuad(images,frameorder,framecolor,frameduration,fixationorder,fixationcolor,fixationsize, ...
%               grayval,detectinput,wantcheck,offset,moviemask,movieflip,scfactor,allowforceglitch, ...
%               triggerfun,framefiles,frameskip,triggerkey,specialcon,trialtask,maskimages,specialoverlay, ...
%               frameevents,framefuncs,setupscript,cleanupscript,stereocontrol,stereoflip,datapixxFlag)
% 
...
% 2020/9/20  - added datapixxFlag to allow Vpixx triggering
% example:
% pton;
...
if ~exist('datapixxFlag','var') || isempty(datapixxFlag)
  datapixxFlag = 0;
end
...
if iscell(fixationsize) && setnum~=135 && setnum~=134 % dot case
  % easy case
  if ~focase4
    if focase3
...
elseif setnum~=135 || setnum~=134 % cross case
    
    fixationrect = CenterRect([0 0 fixationsize{1}(1) fixationsize{1}(1)], rect)+ [offset(1) offset(2) offset(1) offset(2)];
...
if ~iscell(fixationorder)
  ...
  % dot case
  if iscell(fixationsize)  && setnum~=135 && setnum~=134
  ...
  elseif setnum~=135 || setnum~=134 % cross case
      LineWidth_Pix=fixationsize{1}(1)/6;
     [CrossMask] = uint8(MyMakeCross(fixationsize{1}(1),LineWidth_Pix));

    for i=1:size(fixationcolor,1)

      fixationimage(:,:,:,i) = cat(3, CrossMask*fixationcolor(i,1), CrossMask.*fixationcolor(i,2) , CrossMask.*fixationcolor(i,3));

    end
    fixationalpha = CrossMask;
...
datapixxFlag=1
if datapixxFlag==1 %NYUAD
   fprintf('Waiting for Scanner Trigger via Vpixx (consider turning off network, energy saver, software updates.)\n');
